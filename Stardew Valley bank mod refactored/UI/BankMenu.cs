...
internal class BankMenu : IClickableMenu
{
    ...
    private readonly TouchOverlay? _touchOverlay;
        ...
        )
    {
        ...
        if (TouchOverlay.ShouldShow(config))
        {
            ...
            _touchOverlay.OnTabRight += () =>
            {
                int maxVisible = _isChinese ? 4 : 3;
                if (_tabScrollOffset < Math.Max(0, _companyList.Count - maxVisible)) { _tabScrollOffset++; Game1.playSound("smallSelect"); }
            };
            _touchOverlay.OnScroll += direction => receiveScrollWheelAction(direction);
            _touchOverlay.OnCancel += () => exitThisMenu();
        }
    }
    ...
    public override void draw(SpriteBatch b)
    {
        ...
        if (_isChinese)
        {
            ...
            {
                string cl; Color cc;
                if (!_account.UseCompoundInterest)
                { cl = I18n.Get("uib.104"); cc = Color.DimGray; }
                else if (!string.IsNullOrEmpty(_account.CompoundActiveCompany) && _account.CompoundActiveCompany == CurrentCompany.Name)
    ...
    public override void receiveLeftClick(int x, int y, bool playSound = true)
    {
        ...
        if (_depositBtn.containsPoint(x, y))
        {
            ...
            Game1.activeClickableMenu = new NumberInputMenu(
                ...
                config: _config, monitor: _services.Monitor
            );
            return;
        }
        ...
        // Stage 15: compound interest button (per-save toggle)
        if (_compoundBtn.containsPoint(x, y))
        {
            if (!_account.UseCompoundInterest)
            {
                // Enable compound for this save and activate on current company
                _account.UseCompoundInterest = true;
                if (_account.CompoundCooldownDays <= 0 && string.IsNullOrEmpty(_account.CompoundActiveCompany))
                {
                    _account.CompoundActiveCompany = CurrentCompany.Name;
                    _account.CompoundDaysRemaining = _account.CompoundDurationDays;
                }
                _accountService.Save(_account);
                Game1.chatBox?.addInfoMessage(I18n.Get("uib.105"));
            }
            else if (!string.IsNullOrEmpty(_account.CompoundActiveCompany) && _account.CompoundActiveCompany == CurrentCompany.Name)
            {
                // Toggle off compound for this company
            ...
            {
                // Activate compound on current company
                _account.CompoundActiveCompany = CurrentCompany.Name;
                _account.CompoundDaysRemaining = _account.CompoundDurationDays;
                _accountService.Save(_account);
