...
internal class NumberInputMenu : IClickableMenu
{
    ...
    public NumberInputMenu(string prompt, Action<int> onConfirm, Action onCancel,
                           ...
                           ModConfig? config = null, StardewModdingAPI.IMonitor? monitor = null)
        : base(
        ...
        )
    {
        ...
        _onCancel = onCancel;
        _confirmLabel = confirmLabel ?? I18n.Get("uiin.1");
        _cancelLabel = cancelLabel ?? I18n.Get("uib.86");
        ...
        if (config is not null && monitor is not null && TouchOverlay.ShouldShow(config))
        {
            _touchOverlay = new TouchOverlay(config, monitor);
