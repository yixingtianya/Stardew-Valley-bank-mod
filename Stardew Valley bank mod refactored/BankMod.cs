    ...
    *********/
    ...
    private void RegisterModConfigOptions(IGenericModConfigMenuApi gmcm)
    {
        ...
        gmcm.AddBoolOption(ModManifest,
            ...
            setValue: val =>
            {
                _config.UseCompoundInterest = val;
                // Sync to per-save data if a save is loaded
                if (Context.IsWorldReady)
                {
                    var acc = _services.BankAccountService.Load();
                    acc.UseCompoundInterest = val;
                    _services.BankAccountService.Save(acc);
                }
            },
        ...
        gmcm.AddNumberOption(ModManifest,
            ...
            setValue: val =>
            {
                _config.CompoundDurationDays = val;
                if (Context.IsWorldReady)
                {
                    var acc = _services.BankAccountService.Load();
                    acc.CompoundDurationDays = val;
                    _services.BankAccountService.Save(acc);
                }
            },
    ...
    private void OnSaveLoaded(object? sender, SaveLoadedEventArgs e)
    {
        ...
        // Migration: initialize per-save compound settings from GMCM config (first load only)
        if (startAccount.CompoundConfigVersion < 1)
        {
            startAccount.UseCompoundInterest = _config.UseCompoundInterest;
            startAccount.CompoundDurationDays = _config.CompoundDurationDays;
            startAccount.CompoundConfigVersion = 1;
            _services.BankAccountService.Save(startAccount);
            Monitor.Log($"[Migration] Initialized compound settings from config: UseCompound={startAccount.UseCompoundInterest}, Duration={startAccount.CompoundDurationDays}", LogLevel.Info);
        }
    ...
    private void OnButtonPressed(object? sender, ButtonPressedEventArgs e)
    {
        ...
        // Support configurable key + default right-click/controller + mobile touch
