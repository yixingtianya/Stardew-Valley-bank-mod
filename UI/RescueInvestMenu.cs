...
internal class RescueInvestMenu : IClickableMenu
{
    ...
    public override void draw(SpriteBatch b)
    {
        ...
        string title = $"入股救市 — {_companyName}";
        Vector2 titleSize = Game1.dialogueFont.MeasureString(title);
        Utility.drawTextWithShadow(b, title, Game1.dialogueFont,
            new Vector2(centerX - titleSize.X / 2, yPositionOnScreen + 20), Game1.textColor);

        string sub = $"每百股 {_sharePrice:N0} g = 1 天（已购 {_alreadyPurchased}/7 百股）";
        ...
        string countStr = $"{_sharesToBuy} 百股";
        ...
        string costStr = $"需支付：{totalCost:N0} g";
