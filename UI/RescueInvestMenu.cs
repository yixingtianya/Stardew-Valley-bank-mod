...
internal class RescueInvestMenu : IClickableMenu
{
    ...
    public override void draw(SpriteBatch b)
    {
        ...
        int contentTop = yPositionOnScreen + 80;
        int curY = contentTop;
        ...
        Utility.drawTextWithShadow(b, title, Game1.smallFont,
            new Vector2(centerX - titleSize.X / 2, curY), Game1.textColor);
        curY += (int)titleSize.Y + 12;

        // Subtitle — auto-wrap
        string sub = I18n.Get("uir.2", new { price = _sharePrice, days = 7, shares = _alreadyPurchased });
        var subLines = WrapText(Game1.smallFont, sub, width - 80);
        float subLineH = Game1.smallFont.LineSpacing;
        for (int li = 0; li < subLines.Count; li++)
        {
            Vector2 ls = Game1.smallFont.MeasureString(subLines[li]);
            Utility.drawTextWithShadow(b, subLines[li], Game1.smallFont,
                new Vector2(centerX - ls.X / 2, curY + li * (int)subLineH), Color.DarkCyan);
        }
        curY += subLines.Count * (int)subLineH + 20;

        // +/- buttons — symmetric, wide apart
        int btnW = 48;
        int btnH = 42;
        int btnGap = 280;
        _minusRect = new Rectangle(centerX - btnGap / 2 - btnW / 2, curY, btnW, btnH);
        _plusRect = new Rectangle(centerX + btnGap / 2 - btnW / 2, curY, btnW, btnH);
        ...
        // Share count — centered between buttons
        string countStr = I18n.Get("uir.3", new { shares = _sharesToBuy });
        Vector2 countSize = Game1.smallFont.MeasureString(countStr);
        Utility.drawTextWithShadow(b, countStr, Game1.smallFont,
            new Vector2(centerX - countSize.X / 2, curY + (btnH - countSize.Y) / 2), Color.Gold);
        curY += btnH + 16;
        ...
        Utility.drawTextWithShadow(b, costStr, Game1.smallFont,
            new Vector2(centerX - costSize.X / 2, curY), Color.Yellow);
